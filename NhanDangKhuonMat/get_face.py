...
def run():
    ...
    name = st.text_input("Tên khuôn mặt",disabled=st.session_state.disabled)
    confirm = st.button("Bắt đầu thêm dữ liệu")
    
    
    if confirm:
        path = "./NhanDangKhuonMat/image/"+name
        ...
        def save(faces):
            ...
            if faces[1] is not None:
                ...
                file_name = path + '/'+"name"+ '_%04d.bmp' % dem
                cv.imwrite(file_name, face_align)
                
                
            
        st.button("Chụp",on_click=save(detector.detect(frame)))
        while True:
            hasFrame, frame = cap.read()
            if not hasFrame:
